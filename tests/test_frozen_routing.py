...
def test_freezing_another_app_reuses_shared_frozen_branches():
    app, _ = make_app()
    app.freeze_routes()
    other = FastAPI()
    other.mount("/shared", app)
    other.freeze_routes()
    assert TestClient(other).get("/shared/api/v1/acme/typed/42").json() == {
        "name": "int",
        "params": {"tenant": "acme", "value": 42},
    }


def test_host_registration_and_freezing():
    hosted, _ = make_app()
    app = FastAPI()
    version = app.router._get_routes_version()
    app.router.host("api.example.com", hosted, name="api")
    assert app.router._get_routes_version() > version
    app.freeze_routes()
    assert hosted.router._routes_frozen
    client = TestClient(app, base_url="http://api.example.com")
    assert client.get("/api/v1/acme/typed/42").status_code == 200
    assert client.get("http://other.example.com/api/v1/acme/typed/42").status_code == 404
...
def test_custom_match_hooks_preserve_calls_and_scope():
    ...
    assert client.get("/api/declared").json() == 2
    app.freeze_routes()
    assert client.get("/surprise").json() == 2
    assert client.get("/api/declared").json() == 2
...
def test_custom_slash_converter(monkeypatch):
    ...
    assert app.url_path_for("endpoint", value="a/b") == "/api/files/a/b/suffix"
